...
# Sort the dataframes
wb_sell_original = wb_sell_original.sort_values(by=['execbroker', 'symbol', 'strikeprice'])
trf_buy_original = trf_buy_original.sort_values(by=['ContraBroker', 'Symbol', 'AvgPx'])

# Extract the symbols from both DataFrames
wb_symbols = wb_sell_original['symbol'].tolist()
trf_symbols = trf_buy_original['Symbol'].tolist()

wb_symbols = set(wb_symbols)
trf_symbols = set(trf_symbols)

# Find the Brokers that are common between both df 
common_brokers = set(wb_sell_original['execbroker'].unique()) & set(trf_buy_original['ContraBroker'].unique())
...
idx_wb = 0 
num_rows_wb = wb_sell_original.shape[0]
idx_trf = 0
num_rows_trf = trf_buy_original.shape[0]
...
# Iterate through wb_sell_original and accumulate prices and quantities
while idx_wb < num_rows_wb:
    wb_row = wb_sell_original.iloc[idx_wb]
...
# Iterate through trf_buy_original and accumulate prices and quantities
while idx_trf < num_rows_trf:
    trf_row = trf_buy_original.iloc[idx_trf]
...
total_trf_rows = trf_buy_original.shape[0]
...
output_dir = 'Cumulative CSV Results WB SELL TRF BUY'
os.makedirs(output_dir, exist_ok=True)
matching_wb_df.to_csv(os.path.join(output_dir, 'wb_sell_trf_buy_matching_wb_CUMULATIVE.csv'), index=False)
matching_trf_df.to_csv(os.path.join(output_dir, 'wb_sell_trf_buy_matching_trf_CUMULATIVE.csv'), index=False)
not_matching_wb_df.to_csv(os.path.join(output_dir, 'wb_sell_trf_buy_not_matching_wb_CUMULATIVE.csv'), index=False)
not_matching_trf_df.to_csv(os.path.join(output_dir, 'wb_sell_trf_buy_not_matching_trf_CUMULATIVE.csv'), index=False)
